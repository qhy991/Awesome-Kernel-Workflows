...
│   ├── manifest.yaml
│   └── README.md
├── HarnessEngineering/          # 冻结 harness 的 profile/实现/官方 gate 循环
│   ├── harness-engineering-kernel-optimization.js
│   ├── manifest.yaml
│   └── README.md
├── Atrex/                       # Atrex Kernel Agent 严格适配器
...
| [Harness Engineering](HarnessEngineering/) | 通过调用方外部 harness 支持 CUDA/Triton/CuTe | `gpu-kernel-optimization`, `harness-constrained-kernel-optimization`, `flashinfer-kernel-optimization` | 已有 kernel 和冻结的编译/正确性/计时命令；基于 profile 的有界修改与完整产物保留 | 缺少可执行 harness 契约；没有 baseline 的生成任务；把文字说明当作验证 |
| [Atrex](Atrex/) | 通过官方 Atrex orchestrator 支持 CUDA/ROCm | `atrex-kernel-optimization`, `production-kernel-optimization`, `sol-execbench-operator-optimization` | 已有 Atrex checkout 和 GPU gateway；SOL-ExecBench 或原生 Atrex-Bench campaign | 缺少官方 checkout/platform/命令；普通项目 harness；绕过 Atrex supervisor |
...
| [Harness Engineering](HarnessEngineering/) | ![CUDA](https://img.shields.io/badge/CUDA-76B900?style=flat&logo=nvidia&logoColor=white) ![Triton](https://img.shields.io/badge/Triton-6C3483?style=flat) ![ExternalHarness](https://img.shields.io/badge/external--harness-teal?style=flat) ![Verification](https://img.shields.io/badge/verification-green?style=flat) | FreezeContract → Baseline → Profile/Decide → Implement → OfficialGate → Audit | [arXiv:2607.17979](https://arxiv.org/abs/2607.17979) / [公开产物](https://github.com/syhya/mlsys26-flashinfer-contest) |
...
| [Harness Engineering](HarnessEngineering/) | `iterative_self_improving` | 冻结契约、profile 支持的单假设轮次、官方 gate | 调用方拥有的编译/正确性/验证/计时命令 | incumbent 证据、接受/拒绝的 strategy id、原始产物 | workflow adaptation；竞赛基础设施和隐藏测试在外部 |
| [Atrex](Atrex/) | `iterative_self_improving` | 把官方 Atrex Long Horizon campaign 委托给唯一 orchestrator | 完整 workload 终局验证 + 同 allocation ABBA | canonical memory/vN、episode journal、worktree、晋升版本 | 严格 adapter；不重复 Atrex supervisor 或优化循环 |
