...
2a. **Runtime & contract anti-patterns (all CI-enforced — a PR with any of these hard-fails).** Avoiding them by construction is cheaper than getting the PR bounced:
   - **No \`Date.now()\`, \`Math.random()\`, or argless \`new Date()\`** anywhere in the workflow. They break Workflow-tool resume (values differ across resumes → cached branches diverge). Enforced by \`_meta/tools/test/kernelagent-runtime-patterns.test.js\` (extend for other workflows) + KerSor catalog forbidden-API scan (marks the workflow \`known_broken\`). Root cause: AWK issue #22, KerSor #31. If you need a unique id, derive it from \`args.run_index\` / \`args.round_index\` / a filesystem counter.
   - **No line-leading ESM \`import\`** — the entrypoint is loaded as a script, not a module. Use \`require\`-style or template-literal composition.
   - **Wrap every \`await agent()\` in \`agentRetry(fn, {retries, allowNull})\`** — a bare \`agent()\` returning null on a transient 429 will crash the workflow via property deref (\`diag.bottleneck_class\`, \`impl.code\`, ...). Enforced by \`_meta/tools/test/agent-retry-guard-lint.test.js\` + \`agent-retry-null-safety.test.js\`. Root cause: AWK #20. Use the scaffold in \`scripts/add-agent-retry-scaffolding.js\` to auto-wrap existing calls.
   - **Substrate CLI: use \`--artifact\` / \`--problem\` / \`--out\`, never \`--kernel\` / \`--test\` / \`--result\`.** Each substrate declares its accepted flags in \`_substrate/backends/<B>/flags.yaml\`; run.sh parsers are generated from that schema. \`--kernel/--test/--result\` are not accepted by cuda/metal/metax/triton substrates (they will \`exit 3\`). Enforced by \`_meta/tools/test/substrate-flags-contract.test.js\` (part (b) is enforcing). Root cause: AWK #25. Template: \`driverSh('run.sh', \\\`--artifact \${buildOut} --problem \${PROBLEM_PATH} --out \${resultPath}\\\`)\`.
   - **All file writes go to \`args.exp_dir\`, not process CWD.** Workflows are dispatched with a specific \`exp_dir\`; writing to CWD pollutes the KerSor session tree and requires \`collect-stray-outputs.sh\` to sweep. Template: \`const OUT = \\\`\${EXP_DIR}/run-\${round}/candidate.py\\\`\`.
   - **manifest \`routing.accepts.problem_type\` must be non-empty and match the workflow's actual capability** (specific: \`cuda-kernel-{optimization,generation}\`; neutral: \`gpu-kernel-*\`; universal: \`kernel-*\`). Empty list = "accepts any" — only use for meta workflows without eligibility axes (e.g. \`in-place-patch\`). KerSor selector filters on this at catalog time.
