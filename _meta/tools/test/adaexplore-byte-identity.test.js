'use strict'
// RE-BASELINE: adaexplore-golden.json was captured from the PRE-RETROFIT
// AdaExplore body at the commit recorded in
// _meta/tools/fixtures/ADAEXPLORE-GOLDEN-BASELINE-SHA.txt. Do NOT regenerate
...
//   (1) git worktree the SHA in ADAEXPLORE-GOLDEN-BASELINE-SHA.txt,
...
// Stage-A commit SHA: 77197bb28f90650d39c0226918d5666c765752a1
...
const ARGS = load('adaexplore-args.json')
const RETURNS = load('adaexplore-agent-returns.json')
const GOLDEN = load('adaexplore-golden.json')

// Expected label sequence — both expand_large (propose-1) and expand_small
// (revise-2 + tune-2) branches must fire. No adapt-memory: memory_update=false.
...
test('adaexplore legacy path: rendered label set matches the enumerated MCTS unlock sequence', async () => {
  ...
  assert.deepEqual(captured.map(c => c.label), EXPECTED_LABELS,
    'label set drifted — agentReturns map must unlock setup, large-step (step 0 forced large at root), ' +
    'small-step expand (revise+tune), eval at each iteration, and final-report. No adapt-memory in fixture.')
  assert.deepEqual(captured.map(c => c.phase), EXPECTED_PHASES,
    'phase tags drifted from MCTS phase sequence.')
})

test('adaexplore legacy path: prompt sequence is byte-identical to pre-retrofit golden', async () => {
  const captured = await capturePrompts({ workflowPath: WORKFLOW, args: ARGS, agentReturns: RETURNS })
...
test('adaexplore legacy path: capturePrompts is deterministic across repeated invocations', async () => {
...
test('adaexplore legacy path: stable-stringified capture is byte-identical to frozen golden file (UPDATE_GOLDEN=1 to regenerate)', async () => {
  const goldenPath = path.join(FIX, 'adaexplore-golden.json')
  ...
  assert.strictEqual(captured, golden, 'pre-retrofit adaexplore golden drifted; re-run with UPDATE_GOLDEN=1 only if intentional')
})

test('re-baseline SHA file is present and non-empty', () => {
  const sha = fs.readFileSync(path.join(FIX, 'ADAEXPLORE-GOLDEN-BASELINE-SHA.txt'), 'utf8').trim()
  assert.match(sha, /^[0-9a-f]{40}$/, 'SHA file must contain a full 40-char git commit hash')
  const me = fs.readFileSync(__filename, 'utf8')
  assert.ok(me.includes(sha), 'this test file must cite the SHA from ADAEXPLORE-GOLDEN-BASELINE-SHA.txt for human-readable provenance')
})
