...
│   ├── manifest.yaml
│   └── README.md
├── AutoMegaKernel/              # Adapter to AMK megakernel schedule search
│   ├── automegakernel-megakernel-optimization.js
│   ├── manifest.yaml
│   └── README.md
├── HarnessEngineering/          # Frozen-harness profile/implement/official-gate loop
│   ├── harness-engineering-kernel-optimization.js
│   ├── manifest.yaml
│   └── README.md
├── Atrex/                       # Strict adapter to Atrex Kernel Agent
│   ├── atrex-kernel-optimization.js
│   ├── manifest.yaml
│   └── README.md
...
│   └── README.md
├── GemmPTX/                     # GEMM instruction-path optimization via PTX/SASS evidence
...
|----------|----------------------------|---------------------------------|----------|------------|
| [AscendC](AscendC/) | AscendC / Ascend (msprof + `ascendc_direct_launch`) | `ascend-kernel-optimization`, `ascend-kernel-generation` | Ascend 910B NPU kernel optimization/generation via msprof and the substrate ascend backend | CUDA/Triton/ROCm/Metal targets — use their own workflows |
...
| [KDA](KDA/) | CUDA (default) · Triton via driver (experimental) · Ascend via substrate (faithful but simplified) | `cuda-kernel-optimization`, `cuda-kernel-generation`, `ascend-kernel-optimization`, `ascend-kernel-generation` | Evidence-driven CUDA implementation, validation, and optimization loops; Ascend routes through the substrate ascend backend | Backends without a substrate driver or validation contract |
...
| [AutoMegaKernel](AutoMegaKernel/) | CUDA via AutoMegaKernel (method-intrinsic; external harness) | `amk-schedule-search`, `megakernel-synthesis`, `llama-megakernel-optimization` | Existing AMK checkout; Llama-family CUDA megakernel schedule search over `ScheduleConfig` + `kernel_knobs` | Ordinary standalone CUDA/Triton/CUTLASS kernels; missing AMK checkout; high-batch serving throughput |
| [Harness Engineering](HarnessEngineering/) | CUDA/Triton/CuTe via caller-owned external harness | `gpu-kernel-optimization`, `harness-constrained-kernel-optimization`, `flashinfer-kernel-optimization` | Existing kernel plus frozen compile/correctness/timing commands; profile-backed bounded edits with complete artifact retention | Missing executable harness contract; generation without a baseline; treating prose as verification |
| [Atrex](Atrex/) | CUDA/ROCm through the official Atrex orchestrator | `atrex-kernel-optimization`, `production-kernel-optimization`, `sol-execbench-operator-optimization` | Existing Atrex checkout and GPU gateway; SOL-ExecBench or native Atrex-Bench campaign | Missing official checkout/platform/command; ordinary project harnesses; bypassing Atrex's supervisor |
...
| [GemmPTX](GemmPTX/) | CUDA/CuTe/CUTLASS (vendor-locked: PTX/SASS disassembly) | `gemm-ptx-optimization`, `cuda-gemm-ptx-optimization`, `gemm-instruction-optimization` | Existing GEMM kernels where instruction-path selection must be proven by PTX/SASS evidence | Generic compute-bound kernels; missing disassembly/benchmark/correctness contract |
...
| [Generalist](Generalist/) | CUDA/Triton/MetaX (default) · Ascend via substrate (faithful but simplified) | `cuda-kernel-generation`, `cuda-kernel-optimization`, `ascend-kernel-generation`, `ascend-kernel-optimization` | Backend-clean substrate solver (beam); the default single-solver baseline; Ascend via the substrate ascend backend | Tasks needing a non-substrate measurement signal |
...
| [Harness Engineering](HarnessEngineering/) | ![CUDA](https://img.shields.io/badge/CUDA-76B900?style=flat&logo=nvidia&logoColor=white) ![Triton](https://img.shields.io/badge/Triton-6C3483?style=flat) ![ExternalHarness](https://img.shields.io/badge/external--harness-teal?style=flat) ![Verification](https://img.shields.io/badge/verification-green?style=flat) | FreezeContract → Baseline → Profile/Decide → Implement → OfficialGate → Audit | [arXiv:2607.17979](https://arxiv.org/abs/2607.17979) / [artifact](https://github.com/syhya/mlsys26-flashinfer-contest) |
| [Atrex](Atrex/) | ![CUDA](https://img.shields.io/badge/CUDA-76B900?style=flat&logo=nvidia&logoColor=white) ![ROCm](https://img.shields.io/badge/ROCm-ED1C24?style=flat&logo=amd&logoColor=white) ![Profiling](https://img.shields.io/badge/profiling-green?style=flat) ![ExternalHarness](https://img.shields.io/badge/external--harness-teal?style=flat) ![ABBA](https://img.shields.io/badge/ABBA--verification-darkblue?style=flat) | Doctor → LaunchOfficialCampaign → AuditCanonicalEvidence → Report | [arXiv:2607.14541](https://arxiv.org/abs/2607.14541) / [Atrex Kernel Agent](https://github.com/alibaba/atrex-kernel-agent) |
...
|----------|------------------|-----------------|------------------------|------------------------|-------------------|
| [AscendC](AscendC/) | `iterative_self_improving` | plan→generate→evaluate→optimize (msprof) loop | msprof bottleneck class + eval_single_runner timing (ascendc_direct_launch) | candidate submissions, best path, stagnation counter | Substrate reference — the substrate ascend backend owns the measurement signal |
...
| [AutoMegaKernel](AutoMegaKernel/) | `search_based` | Structured ScheduleConfig + kernel_knobs search delegated to AMK loop/autoresearch | AMK validate-before-launch, full-model correctness, latency/roofline verdict | Best config, rows/results.tsv, flywheel corpus | Strict adapter to external AMK harness; not standalone reimplementation |
| [Harness Engineering](HarnessEngineering/) | `iterative_self_improving` | Frozen contract, profile-backed single-hypothesis rounds, official gate | Caller-owned compile/correctness/verification/timing commands | Incumbent evidence, accepted/rejected strategy ids, raw artifacts | Workflow adaptation; contest infrastructure and hidden tests are external |
| [Atrex](Atrex/) | `iterative_self_improving` | Official Atrex Long Horizon campaign delegated to its sole orchestrator | Complete-workload terminal validation plus same-allocation ABBA | Canonical memory/vN, episode journals, worktrees, promoted version | Strict adapter; no duplicate Atrex supervisor or optimization loop |
...
| [GemmPTX](GemmPTX/) | `iterative_self_improving` | One-candidate-at-a-time instruction hypothesis loop with PTX/SASS verification before profiling | compile/test/benchmark/disassemble commands, optional NCU/profile metrics | candidate history, verified instruction paths, hypothesis_not_realized dead ends | Original AKW workflow adaptation for GEMM instruction-path tuning |
