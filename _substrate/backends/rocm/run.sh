#!/usr/bin/env bash
# rocm/run.sh — load the compiled .so, run vs problem, measure correctness + latency.
...
# --- GPU path (deferred): the python launcher does the real torch run on ROCm. ---
...
result = {
    ...
    "error": "GPU execution deferred: requires AMD device + ROCm runtime",
}
try:
    import torch
    if not torch.cuda.is_available():
        # torch.cuda.is_available() returns True for ROCm builds of PyTorch
        result["error"] = "no ROCm/HIP device available (deferred GPU tier)"
