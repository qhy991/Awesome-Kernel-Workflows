...
Return benchmark results.`, {
      ...
      if (USE_DRIVER_STANDALONE) {
        ...
        const runOut = await agentRetry(() => agent(
          ...
          { model: MODEL.profile, label: `driver-run-${envIdx}`, phase: 'Iterate', schema: JSON_PASSTHROUGH }), { retries: 5 })
        let evidenceOut
        if (PROFILING_DECISION.method === 'native_profiler') {
          ...
          evidenceOut = await agentRetry(() => agent(
            ...
            { model: MODEL.mechanical, label: `driver-to-evidence-${envIdx}`, phase: 'Iterate', schema: JSON_PASSTHROUGH }), { retries: 5 })
        } else {
        ...
        const diagOut = await agentRetry(() => agent(
          ...
          { model: MODEL.mechanical, label: `driver-diagnose-${envIdx}`, phase: 'Iterate', schema: JSON_PASSTHROUGH }), { retries: 5 })
        await agentRetry(() => agent(
        ...
        if (INTEGRATION_DECISION.method === 'embedded_inplace' && ORIGINAL_BACKUP) {
          const embResult = await agentRetry(() => agent(
            ...
            { model: MODEL.mechanical, label: `embedded-inplace-${iterCount}`, phase: 'Iterate', schema: JSON_PASSTHROUGH }), { retries: 5 })
          ...
          if (_plan) {
            const embResult = await agentRetry(() => agent(
              ...
              { model: MODEL.mechanical, label: `embedded-dispatch-${iterCount}`, phase: 'Iterate', schema: JSON_PASSTHROUGH }), { retries: 5 })
            embLatency = Number(embResult?.latency_ms || 0)
            embBclass = embResult?.heuristic_bclass || 'unknown'
            embMetrics = embResult?.metrics || { latency_ms: embLatency }
          }
