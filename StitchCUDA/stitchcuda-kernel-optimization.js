...
{"workflow":"${WORKFLOW_NAME}","phase":"Code","ts":"<ts>","status":"done","candidate_id":"attempt-${attempt}","technique":"<the main optimization you implemented this attempt>","note":"<kernel name + key implementation choices, one line>"}`,
    ...
    if (SOL_AVAILABLE) {
      __hostMeasured = await __solExecbenchEvaluate({
        ...
        kernelSource: `${SOL_SEED_DIR || WORKSPACE}/stitch_attempt_${attempt}.cu`,
        candidateSource: (currentCode && currentCode.kernel_code) || '',
        contractEnv: `${SOL_SEED_DIR || '.'}/contract.env`,
        solutionOut: `${SOL_SEED_DIR || WORKSPACE}/stitch_attempt_${attempt}.solution.json`,
        benchOut: `${SOL_SEED_DIR || WORKSPACE}/stitch_attempt_${attempt}.bench.jsonl`,
